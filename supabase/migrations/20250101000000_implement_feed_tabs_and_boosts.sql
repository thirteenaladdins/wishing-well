-- Implement feed tabs (Hot/New/Top) and secure boost functionality
-- This migration adds the new wishes_boosts table and views for different sorting

-- ---------- Update wishes table structure ----------
...
CREATE TABLE IF NOT EXISTS public.wishes_boosts (
  ...
  who TEXT,                       -- anon id / cookie / ip hash (optional)
...
-- ---------- Views: Hot / Top / New ----------
-- Hot = time-decayed score with 48h half-life
...
-- ---------- Rising view (boosts in last 24h) ----------
...
-- Allow public read access to wishes_boosts for analytics
CREATE POLICY "Allow public read wishes_boosts" ON public.wishes_boosts
  FOR SELECT
  USING (true);

-- Prevent direct writes to wishes_boosts (only via boost_wish function)
-- No policy means writes are blocked under RLS
